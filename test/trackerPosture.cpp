...
int main( int argc, char** argv )
{
    ...
    /** 
    */
    cv::Mat frameP45; // rotate 45 degree
    cv::Mat rotateMat1 = cv::getRotationMatrix2D( cv::Point( boundingBox.x + boundingBox.width / 2, boundingBox.y + boundingBox.height / 2 ), 45, 1 );
    cv::warpAffine( frame, frameP45, rotateMat1, frame.size() );

    cv::Mat frameN45; // rotate 45 degree
    cv::Mat rotateMat2 = cv::getRotationMatrix2D( cv::Point( boundingBox.x + boundingBox.width / 2, boundingBox.y + boundingBox.height / 2 ), -45, 1 );
    cv::warpAffine( frame, frameN45, rotateMat2, frame.size() );

    cv::Rect2d bb = boundingBox;
    for(int i = 0; i < 3; i++){     
        tracker->estimateOnly( frameN45, boundingBox );
        tracker->estimateOnly( frame, boundingBox );
        tracker->estimateOnly( frameP45, boundingBox );
    }
    boundingBox = bb;
